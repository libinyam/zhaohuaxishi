...
function cookieSessionId(req) {
  ...
  if (!item) return null;
  // 恶意构造的百分号编码（如 zhsx_session=%）会让 decodeURIComponent 抛 URIError，按无会话处理（issue #51）
  try { return decodeURIComponent(item.slice(item.indexOf('=') + 1)); }
  catch { return null; }
}
...
export function createOAuth() {
  ...
  function session(req, res) {
    ...
    if (!current) {
      // 惰性清扫 + 上限兜底：无 Cookie 请求也会建会话，防扫描流量把 Map 撑爆（会话丢失仅要求重新登录）
      if (sessions.size >= 500) {
        const cutoff = Date.now() - SESSION_MAX_AGE * 1000;
        for (const [k, s] of sessions) if (!s.lastSeen || s.lastSeen < cutoff) sessions.delete(k);
        // 仍超限：只淘汰最久未活动的一个会话（issue #51：原先 clear() 会把全体在线用户同时登出）
        if (sessions.size >= 500) {
          let oldestKey = null;
          let oldestSeen = Infinity;
          for (const [k, s] of sessions) {
            if ((s.lastSeen || 0) < oldestSeen) { oldestSeen = s.lastSeen || 0; oldestKey = k; }
          }
  ...
  async function handleCallback(req, res, url) {
    ...
    current.token = token;
    // expires_in 缺失/非数字：回退到会话上限 TTL——原先 expiresAt=null 会被 authorized() 当作永不过期（issue #51）
    current.expiresAt = Number.isFinite(expiresIn) ? Date.now() + expiresIn * 1000 : Date.now() + SESSION_MAX_AGE * 1000;
  ...
  // 已授权用户的 uid（追问限流的身份标识：OAuth uid 覆盖匿名 UUID）
  function currentUid(req, res) {
    const current = session(req, res);
    return authorized(current) && current.profile?.uid ? `u_${current.profile.uid}` : null;
  }

  return { status, loginUrl, handleCallback, logout, fetchMyFavorites, currentUid, sessionCount: () => sessions.size };
}

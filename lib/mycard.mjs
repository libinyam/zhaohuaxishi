...
// 限流：每用户每天 3 张（仅成功计入）+ 每天 6 次尝试上限（#40）；全站每天 20 张（MYCARD_GLOBAL_CAP 可覆盖），任务发起即写台账、失败不退（防刷）
...
const PER_USER_DAILY = 3; // 每用户每天手动炼卡上限（仅成功计入，失败可重试；演示日评委想连炼几张不被 1 张拦住）
const PER_USER_ATTEMPTS_DAILY = 6; // 每用户每日尝试上限（发起即计、失败不退）：3 张成功 + 3 次失败重试余量，重试也烧全站名额，不能无限（#40）
...
// 契约：FavTime 是秒级 Unix 时间戳（知乎收藏 API 口径，见 lib/report-core.mjs），与 now 同单位比较
...
export function createMyCard(root) {
  ...
  // 台账与 ask/breakdown 共用 data/runtime/quota/<yyyy-mm-dd>.json：{ count（直答总账）, mycards（今日炼卡发起数）, mytries（每用户今日尝试数）, mydone（每用户今日手动成功数） }，字段向后兼容
  ...
  async function readQuota() {
    try {
      ...
      return { date: cstDateStr(), count: 0, mycards: 0, mytries: {}, mydone: {}, ...q };
    } catch { return { date: cstDateStr(), count: 0, mycards: 0, mytries: {}, mydone: {} }; }
  }
  ...
  async function runJob(job) {
    ...
    if (judged.faithful && judged.coreCovered) {
      ...
      // 手动成功计每用户每日 3 张（auto 走自己的 per 配额，不占手动入口）；计数写失败不翻盘已炼成的卡
      if (job.via !== 'auto') {
        try {
          await withQuotaLock(async () => {
            const q = await readQuota();
            q.mydone[job.uid] = (q.mydone[job.uid] || 0) + 1;
            q.updatedAt = Date.now();
            await writeQuota(q);
          });
        } catch (e) { console.error('[mycard] mydone 计数写失败（限流会偏宽松）：', e); }
      }
  ...
  // GET /api/my/card：内存任务优先（只认当天，跨天的内存任务视同年鉴过期，#38），其次今日落盘结果，都没有则 idle + 名额余量
  // quota：used/cap = 全站手动名额，mine/myCap = 每用户今日已炼/上限——前端据此显示「今天已炼 N/3 张」与「再炼一张」
  async function statusFor(uid) {
    const quotaBody = async () => {
      const q = await readQuota();
      return { used: q.mycards, cap: GLOBAL_DAILY_CAP, mine: q.mydone[uid] || 0, myCap: PER_USER_DAILY };
    };
    const job = jobs.get(uid);
    if (job && job.date === cstDateStr()) {
      if (['done', 'failed'].includes(job.status)) return { ...publicJob(job), quota: await quotaBody() };
    ...
    try {
      ...
      if (rec.date === cstDateStr() && rec.via !== 'auto') {
        if (rec.status === 'done') return { ok: true, status: 'done', card: publicCard(rec.card), source: rec.source, quota: await quotaBody() };
        if (rec.status === 'failed') return { ok: true, status: 'failed', error: publicError(rec.error), source: rec.source, quota: await quotaBody() };
      }
    } catch { /* 无记录 */ }
    return { ok: true, status: 'idle', quota: await quotaBody() };
  }

  // POST /api/my/card：运行中→already → 每用户每日 3 张上限（仅成功计入）→ 全局名额（发起即计）→ 无收藏空态 → 发起异步任务
  ...
  async function startInner(uid, getFavorites) {
    ...
    // 注意：done 不再挡再次发起——每用户每天可手动炼 PER_USER_DAILY 张，重复点击 = 炼下一张（下方 mydone 闸门兜上限）
    ...
    // 每用户每日手动上限（仅成功计入；via='auto' 不占）：达上限回最近一张 + dailyCapReached，拉收藏之前就短路
    const qNow = await readQuota();
    if ((qNow.mydone[uid] || 0) >= PER_USER_DAILY) {
      try {
        const rec = JSON.parse(await readFile(resultFile(uid), 'utf8'));
        if (rec.status === 'done' && rec.card) {
          return {
            code: 200,
            body: {
              ok: true, status: 'done', already: true, dailyCapReached: true, card: publicCard(rec.card), source: rec.source,
              quota: { used: qNow.mycards, cap: GLOBAL_DAILY_CAP, mine: qNow.mydone[uid], myCap: PER_USER_DAILY },
            },
          };
        }
      } catch { /* 无落盘记录则落回 429 */ }
      return { code: 429, body: { ok: false, dailyCapReached: true, error: `今天 ${PER_USER_DAILY} 张已炼完，明天再来（订阅后每天 07:00 还会自动炼你没炼过的收藏）` } };
    }
    ...
    // 名额闸门：任务真正发起时写台账，失败不退还（防刷）；但失败不算用户的 3 张之一，可重试（上限见 mytries）
    let gate;
    try {

...
// 异步任务模式：POST 立即返回 jobId，内存 Map 存任务状态，结果落盘 data/cache/mycards/<uid>.json（页面重进可恢复）
// 限流：每用户每天 1 张（仅成功计入）；全站每天 20 张，任务发起即写台账、失败不退（防刷）；失败可重试
...
export function createMyCard(root) {
  ...
  // 台账与 ask/breakdown 共用 data/quota/<yyyy-mm-dd>.json：{ count（直答总账）, mycards（今日炼卡发起数） }，字段向后兼容
  ...
  async function readQuota() {
    try {
      ...
      return { date: cstDateStr(), count: 0, mycards: 0, ...q };
    } catch { return { date: cstDateStr(), count: 0, mycards: 0 }; }
  }
  ...
  // GET /api/my/card：内存任务优先，其次今日落盘结果，都没有则 idle + 全局名额余量
  ...
  async function start(uid, getFavorites) {
    ...
    // 全局名额：任务真正发起时写台账，失败不退还（防刷）；但失败不算用户的 1 张，可重试
  ...
  return { start, statusFor, GLOBAL_DAILY_CAP };
}

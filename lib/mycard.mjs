...
// 异步任务模式：POST 立即返回 jobId，内存 Map 存任务状态（盖 CST 日期戳，跨天自动失效），结果落盘 data/cache/mycards/<uid>.json（页面重进可恢复）
...
function pickFavorite(items) {
  const eligible = (items || []).filter((i) => i && i.Url && i.FavTime && i.ContentType !== 'pin');
...
export function createMyCard(root) {
  ...
  const mycardsDir = path.join(root, 'data', 'cache', 'mycards');
  ...
  // 台账与 ask/breakdown 共用 data/quota/<yyyy-mm-dd>.json：{ count（直答总账）, mycards（今日炼卡发起数）, mytries（每用户今日尝试数） }，字段向后兼容
  ...
  function publicJob(job) {
    ...
    if (job.status === 'failed') return { ok: true, jobId: job.jobId, status: 'failed', error: job.error, source };
  ...
  async function ensureBreakdown(item, key) {
    ...
    // 计数前置：即将发请求，无论成败计 1 次（与 breakdown.mjs 同口径）；阈值检查与计数在同一锁内
    await withQuotaLock(async () => {
      const q = await readQuota();
      if (q.count >= ZHIDA_THRESHOLD) throw new Error('今日直答总额度紧张，现场拆解暂停开放，明天再来');
      q.count++;
    ...
    const resp = await fetch('https://developer.zhihu.com/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: ZHIDA_MODEL, messages: [{ role: 'user', content: query }] }),
      signal: AbortSignal.timeout(ZHIDA_TIMEOUT),
    });
    // 上游故障常返回 HTML 错误页：先验状态再 parse，报可读文案（#42）
    if (!resp.ok) throw new Error(`直答服务暂时不可用（HTTP ${resp.status}）`);
    let payload;
    try { payload = await resp.json(); }
    catch { throw new Error(`直答服务返回了无法解析的响应（HTTP ${resp.status}）`); }
    const content = payload?.choices?.[0]?.message?.content;
    if (!content) throw new Error(`直答返回异常（HTTP ${resp.status}）`);
    // 落盘缓存：铁律「同一收藏永不重复请求」，后续炼卡/重试直接命中
    const rec = { key, url: item.Url, title: item.Title, query, model: payload.model, content, fetchedAt: Date.now() };
  ...
  async function statusFor(uid) {
    ...
    try {
      ...
      if (rec.date === cstDateStr()) {
        if (rec.status === 'done') return { ok: true, status: 'done', card: publicCard(rec.card), source: rec.source };
        if (rec.status === 'failed') return { ok: true, status: 'failed', error: rec.error, source: rec.source };
      }
    } catch { /* 无记录 */ }
    const q = await readQuota();
  ...
  async function startInner(uid, getFavorites) {
    ...
    // 今日已成功生成 → 直接返回已有结果（不重复烧额度）
    try {
      const rec = JSON.parse(await readFile(resultFile(uid), 'utf8'));
      if (rec.date === cstDateStr() && rec.status === 'done') {
  ...
  // 同 uid 的 start 串行化（#39：getFavorites 是真网络调用，并发双发曾双双穿过 existing 检查双烧名额；
  // 排队的第二个请求进入 startInner 时必看到第一个的占位/结果，按既有 already 语义返回）
  const inflight = new Map(); // uid -> 链尾 Promise
  function start(uid, getFavorites) {
    const prev = inflight.get(uid) || Promise.resolve();
    const run = prev.then(() => startInner(uid, getFavorites));
  ...
  return { start, statusFor, listCards, markReviewed, GLOBAL_DAILY_CAP };
}

...
export function createGeminiChat({ base, key, model, temperature, timeout = 120000 }) {
  return async function chat(prompt) {
    ...
    if (typeof content !== 'string' || !content) {
      throw new Error(`gemini 响应结构异常：choices[0].message.content 缺失（${JSON.stringify(data).slice(0, 200)}）`);
    }
    return content;
  };
}

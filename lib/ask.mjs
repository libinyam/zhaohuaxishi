...
// 容器时区不可靠，固定 UTC+8 切日（与推送调度同思路）
const cstDate = () => new Date(Date.now() + 8 * 3600 * 1000).toISOString().slice(0, 10);
...
export function createAsk(root) {
  ...
  const quotaFile = () => path.join(quotaDir, `${cstDate()}.json`);
  ...
  async function readQuota() {
    try {
      ...
      return { date: cstDate(), count: 0, asks: {}, ips: {}, ...q };
    } catch { return { date: cstDate(), count: 0, asks: {}, ips: {} }; }
  }
